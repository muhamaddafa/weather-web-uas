...
function App() {
  ...
  useEffect(() => {
    ...
    Geocode.fromAddress(varKota).then(
      (response) => {
        ...
        fetch(`https://open-weather13.p.rapidapi.com/city/latlon/${lat}/${lng}`, options)
        ...
        fetch(`https://open-weather13.p.rapidapi.com/city/fivedaysforcast/${lat}/${lng}`, options)
          .then((response) => response.json())
          .then((response) => {
            let temp0 = response.list[ramalan1].main.temp;
            setTemp0(temp0);
            const waktu0 = response.list[ramalan1].dt_txt.split(" ")[timeSet].slice(slicing[0], slicing[1]);
            setTime0(waktu0);
            ...
            setTemp1(temp1);
            const waktu1 = response.list[ramalan2].dt_txt.split(" ")[timeSet].slice(slicing[0], slicing[1]);
            setTime1(waktu1);
            ...
            setTemp2(temp2);
            const waktu2 = response.list[ramalan3].dt_txt.split(" ")[timeSet].slice(slicing[0], slicing[1]);
            setTime2(waktu2);
            ...
            setTemp3(temp3);
            const waktu3 = response.list[ramalan4].dt_txt.split(" ")[timeSet].slice(slicing[0], slicing[1]);
            setTime3(waktu3);
            ...
            setTemp4(temp4);
            const waktu4 = response.list[ramalan5].dt_txt.split(" ")[timeSet].slice(slicing[0], slicing[1]);
            setTime4(waktu4);
  ...
  }, [forecastPeriod, ramalan1, ramalan2, ramalan3, ramalan4, ramalan5, varKota, satuanSuhu, slicing, timeSet]);
  return (
    <>
      <NavBar />
      <div className="container mx-auto py-5 px-5 lg:px-0 lg:grid lg:grid-cols-12 gap-4 dark:text-white">
        <div className="main col-span-4">
          ...
          <CardForecast temperature={mainData.main?.temp} cityName={mainData.name} description={mainData.weather?.[0].description} timezoneOffset={mainData.timezone} iconWeather={mainData.weather?.[0].icon} satuanSuhu={satuanSuhu} />
        </div>
        <div className="addOn col-span-8">
          <div className="flex lg:mt-0 justify-between h-12 p-1 mb-5">
            <TodayWeekButton
          ...
          <div className="wrapperForecast flex gap-4 overflow-scroll lg:justify-between lg:overflow-visible">
            <div>
              <DayForecast tempFore={tempForecast0} waktu={waktu0} icon={icon0} satuanSuhu={satuanSuhu} />
            </div>
            <div>
              <DayForecast tempFore={tempForecast1} waktu={waktu1} icon={icon1} satuanSuhu={satuanSuhu} />
            </div>
            <div>
              <DayForecast tempFore={tempForecast2} waktu={waktu2} icon={icon2} satuanSuhu={satuanSuhu} />
            </div>
            <div>
              <DayForecast tempFore={tempForecast3} waktu={waktu3} icon={icon3} satuanSuhu={satuanSuhu} />
            </div>
            <div>
              <DayForecast tempFore={tempForecast4} waktu={waktu4} icon={icon4} satuanSuhu={satuanSuhu} />
            </div>
          </div>
          <h1 className="mb-3 text-lg font-semibold ps-1 mt-9">Today's Highlight</h1>
          <div className="Information grid grid-cols-12 gap-4">
            <div className="col-span-6">
              <Highlight icon={humidityIcon} title={humditiyTitle} value={mainData.main?.humidity + "%"} stat={hitungKelembapan(mainData.main?.humidity)} satuan={satuan} />
            </div>
            <div className="col-span-6">
              <Highlight icon={windIcon} title={windTitle} value={mainData.wind?.speed?.toFixed(0) + " km/h"} stat={mainData.wind?.deg + String.fromCharCode(176)} satuan={satuan} />
            </div>
            <div className="col-span-6">
              <Highlight icon={temperatureIcon} title={temperatureTitle} value={hitungMinTemp(mainData.main?.temp_min, satuanSuhu) + String.fromCharCode(176)} stat={"Min.Temp"} satuan={satuanSuhu} />
            </div>
            <div className="col-span-6">
              <Highlight icon={visibilityIcon} title={visibilityTitle} value={mainData.visibility / 1000 + " km"} stat={hitungVisibility(mainData.visibility)} satuan={satuan} />
            </div>
          </div>
        </div>
      </div>
