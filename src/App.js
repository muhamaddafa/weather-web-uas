...
function App() {
	...
	const [windStat, setWindStat] = useState("");
	const [humidityStat, setHumidityStat] = useState("");
	const [minTempStat, setMinTemperatureStat] = useState("");
	const [visibilityStat, setVisibilityStat] = useState("");
	...
	const [hasilMinTemp, setHasilMinTemp] = useState("");
	const satuan = "";
	...
	useEffect(() => {
		...
		fetch(`https://open-weather13.p.rapidapi.com/city/${varKota}`, options)
			.then((response) => response.json())
			.then((response) => {
				...
				const humidVal = response.main.humidity;
				const windVal = response.wind.speed;
				const minTempVal = response.main.temp_min;
				const hasilMinTemp = hitungMinTemp(minTempVal, satuanSuhu);
				const visibilityVal = response.visibility;
				const windStatVal = response.wind.deg;
				const humidityStatVal = hitungRH(humidVal);
				const minTempStatVal = "Min. Temp";
				const visibilityStatVal = hitungVisibility(visibilityVal);
				const weatherNowicon = response.weather[0].icon;
				setWeatherNow(weatherNowicon);
				setTemperature(temp);
				setCityName(name);
				setDescription(desc);
				setTimeZone(timezoneOffset);
				setHumidity(humidVal + "%");
				setHumidityStat(humidityStatVal);
				setWindSpeed(windVal.toFixed(0));
				setWindStat(windStatVal);
				setVisibility(visibilityVal / 1000);
				setVisibilityStat(visibilityStatVal);
				setHasilMinTemp(hasilMinTemp);
				setMinTemperatureStat(minTempStatVal);
			})
			.catch((err) => console.error(err));
	}, [
		forecastPeriod,
			...
			<div className="container mx-auto py-5 px-5 lg:px-0 lg:grid lg:grid-cols-12 gap-4 dark:text-white">
				<div className="main col-span-4">
					...
					<CardForecast
						...
						iconWeather={weatherNow}
						satuanSuhu={satuanSuhu}
					/>
				</div>
				<div className="addOn col-span-8">
					<div className="flex lg:mt-0 justify-between">
						<TodayWeekButton
					...
					<div className="wrapperForecast py-5 flex gap-4 overflow-scroll lg:justify-between lg:overflow-visible">
						<div>
							<DayForecast
					...
					<h1 className="mb-3 text-lg font-semibold ps-1">Today's Highlight</h1>
					<div className="Information grid grid-cols-12 gap-4">
						...
						<div className="col-span-6">
							<Highlight
								...
								stat={windStat + String.fromCharCode(176)}
								satuan={satuan}
							/>
						</div>
						<div className="col-span-6">
							<Highlight
								...
								value={hasilMinTemp + String.fromCharCode(176)}
								stat={minTempStat}
								satuan={satuanSuhu}
							/>
						</div>
